...
contract ImplERC721 is ERC721 {
    ...
    function mint(string calldata _tokenURI) external returns (uint256) {
        ...
        memTokenURI[mintedTokens] = _tokenURI;

        emit Transfer(address(0x0), msg.sender, mintedTokens);
        return mintedTokens;
    }

    function mintFor(string calldata _tokenURI, address _owner)
        external
        returns (uint256)
    {
        require(
        ...
        mintedTokens = mintedTokens + 1;
        tokenOwners[mintedTokens] = _owner;
        balanceOfToken[_owner] = balanceOfToken[_owner] + 1;
        memTokenURI[mintedTokens] = _tokenURI;

        emit Transfer(address(0x0), _owner, mintedTokens);
        return mintedTokens;
    }
        ...
        returns (address)
    {
        require(
            ...
            "_tokenId is not a valid NFT tokenID"
        );
    ...
    function safeTransferInternal(
        ...
        if (isContract(_to)) {
            ...
            require(
                retval == 0x150b7a02,
                "The NFT was not received properly by the contract"
            );
        }
    }
    ...
    function transferInternal(
        ...
        require(
            tokenOwners[_tokenId] == _from,
            "The specified _from does not match the current token owner"
        );
